/* ─────────────────────────────────────────────────────────────────
   ...
   사용처:
     ...
     - 쓰기:   addWatched (VideoDetailPage), removeWatched (HistoryPage)
...
interface HistoryState {
  ...
  /** 영상 시청 기록 추가 (VideoDetailPage에서 호출) */
  ...
  persist(
    (set, get) => ({
      ...
      addWatched: (videoId) => {
        ...
        const withoutDuplicate = watchedVideoIds.filter((e) => e.id !== videoId);
        set({ watchedVideoIds: [entry, ...withoutDuplicate] });
      },
